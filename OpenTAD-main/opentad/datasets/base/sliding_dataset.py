...
class SlidingWindowDataset:
    ...
    def get_dataset(self):
        ...
        for video_name, video_info in anno_database.items():
            ...
            tmp_data_list = self.split_video_to_windows(video_name, video_info, video_anno, box_datas[video_name + ".mp4"])
            self.data_list.extend(tmp_data_list)
        assert len(self.data_list) > 0, f"No data found in {self.subset_name} subset."

    def split_video_to_windows(self, video_name, video_info, video_anno, bbox):
